...
class GlobalBanCog(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot):
        ...
        # grupo /gban (criado, mas registrado no cog_load)
        self.gban = app_commands.Group(name="gban", description="Comandos de ban global")
        self._register_slash()   # adiciona sub-comandos ao grupo
    ...
    # -------- util log --------
    async def _send_log(self, g: discord.Guild, e: discord.Embed):
        ch_id = self.log_channels.get(g.id)
        ch = g.get_channel(ch_id) if ch_id else g.system_channel
        if ch and ch.permissions_for(g.me).send_messages:
            await ch.send(embed=e)

    async def _broadcast(self, e: discord.Embed, guilds: List[discord.Guild]):
        await asyncio.gather(*(self._send_log(g, e) for g in guilds))
