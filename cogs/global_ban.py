...
class GlobalBanCog(commands.Cog):
    ...
    gban_slash = app_commands.Group(name="gban", description="Comandos de ban global")
    ...
    async def _send_in_log(self, guild: discord.Guild, embed: discord.Embed):
        ...
        ch = guild.get_channel(chan_id) if chan_id else guild.system_channel
        if ch and ch.permissions_for(guild.me).send_messages:
            try: await ch.send(embed=embed)
            except Exception: logger.exception("Falha log em %s", guild.name)

    async def _broadcast(self, embed, guilds):  # type: ignore
        for g in guilds: await self._send_in_log(g, embed)
    ...
    async def _exec_ban(self, user, mod, reason):
        ...
        emb = E.ok(msg, footer=f"Banido por {mod}", thumbnail_url=user.display_avatar.url)
    ...
    async def gban_p(self, ctx, target: discord.User, *, reason="Sem Motivo"):
        try:  emb = await self._exec_ban(target, ctx.author, reason)
        except RuntimeError as e: return await ctx.send(embed=E.err(str(e)))
    ...
    async def gban_add(self, inter, target: discord.User, reason: str = "Sem Motivo"):
        ...
        try:  emb = await self._exec_ban(target, inter.user, reason)
    ...
    async def gban_list(self, inter, page: int = 1):
        ...
        per, pages = 10, (len(bans)+9)//10; page = max(1, min(page, pages))
        def make(i):
            body = "\n".join(f"`{b.discord_id}` • {b.reason} • <t:{int(b.timestamp.timestamp())}:R>"
                             for b in bans[i*per:(i+1)*per])
            return E.info(body, footer=f"Pág {i+1}/{pages}")

        class P(discord.ui.View):
            def __init__(self, i=page-1): super().__init__(timeout=60); self.i=i
            @discord.ui.button(label="◀️") async def p(self,b,i): 
                if self.i: self.i-=1; await i.response.edit_message(embed=make(self.i), view=self)
            @discord.ui.button(label="▶️") async def n(self,b,i): 
                if self.i<pages-1: self.i+=1; await i.response.edit_message(embed=make(self.i), view=self)

        await inter.response.send_message(embed=make(page-1), view=P(), ephemeral=True)
    ...
    async def cog_command_error(self, ctx, error):
        if isinstance(error, commands.NotOwner):
            await ctx.send(embed=E.err("Somente o dono usa este comando."))
        else:
